...
def sanitize_html(html):
    allowed_tags = markdown_tags + ['h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'span']
    allowed_attrs = markdown_attrs.copy()
    allowed_attrs.update({'span': ['style', 'class'], 'div': ['class']})
...
class VaLogGenerator:
    def __init__(self):
        ...
        self.md = markdown.Markdown(extensions=[
            ...
            'codehilite': {'linenums': True, 'guess_lang': False, 'pygments_style': 'github'},
            'toc': {'permalink': True, 'baselevel': 2}
        }, output_format='html5')

    def extract_metadata_and_body(self, body):
        if not body:
            return {"summary": "", "vertical_title": "", "body": ""}
        ...
        for i in range(min(len(lines), 3)):
            ...
            if line.startswith('!vml-'):
                ...
                if match:
                    ...
                    if 'summary' in line or i == 0: summary = content
                    if 'title' in line or i == 1: vertical_title = content
    ...
    def run(self):
        ...
        for cid in cached_ids:
            if cid not in remote_ids:
                print(f"同步删除: Issue #{cid} 已在远程关闭/移除")
        ...
        for issue in issues:
            ...
            if need_update:
                ...
                try:
                    ...
                    print(f"文件写入失败 #{iid}: {e}")
                
                self.cache[iid] = updated_at
            else:
                print(f"保持原样: #{iid} (无变动)")
        ...
        base_data = {
            ...
            "articles": sorted(all_articles, key=lambda x: x['date'], reverse=True),
            "specials": specials,
